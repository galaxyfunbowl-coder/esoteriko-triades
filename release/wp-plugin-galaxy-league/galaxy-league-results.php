...
define('GLR_VER', '0.3.0');
define('GLR_DIR', plugin_dir_path(__FILE__));
define('GLR_URL', plugin_dir_url(__FILE__));

$need = ['inc/DB.php','inc/Admin.php','inc/Standings.php','inc/Ajax.php','inc/Logic.php','inc/Api.php'];
foreach ($need as $rel) {
  $path = GLR_DIR . $rel;
  if (!file_exists($path)) {
    add_action('admin_notices', function() use ($rel) {
      echo '<div class="notice notice-error"><p>Galaxy League Results: Missing file <code>'.esc_html($rel).'</code>.</p></div>';
    });
    return;
  }
  require_once $path;
...
class GalaxyLeagueResults {
  ...
  public function admin_menu() {
    ...
    add_submenu_page('glr-settings','Manage','Manage','manage_options','glr-manage',['GLR_Admin','render_manage_page']);
    add_submenu_page('glr-settings','Setup Wizard','Setup Wizard','manage_options','glr-setup',['GLR_Admin','render_setup_page']);
  }
