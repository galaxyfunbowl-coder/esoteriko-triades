...
const api = (p, opt={}) => fetch(GLR.rest + p, {
...
function SeasonsTab(){
  ...
  const refresh=()=>api('seasons').then(j=>setSeasons(j.rows||[]))
  useEffect(()=>{refresh()},[])
  const save=async()=>{ await api('seasons',{method:'POST',body:JSON.stringify(form)}); setForm({...form,name:''}); refresh() }
  return (
    <>
      <h2>Seasons</h2>
      <div style={row}>
...
function MatchDaysEditor({seasons}){
  ...
  useEffect(()=>{ if(seasonId) api(`match-days?season_id=${seasonId}`).then(j=>setRows(j.rows||[])) },[seasonId])
  const reload=()=>{ if(seasonId) api(`match-days?season_id=${seasonId}`).then(j=>setRows(j.rows||[])) }
  const add=async()=>{ await api('match-days',{method:'POST',body:JSON.stringify({...form,season_id:Number(seasonId)})}); reload() }
  const del=async(id)=>{ await api(`match-days/${id}`,{method:'DELETE'}); reload() }
  return (
    <>
      <h2>Match Days</h2>
      <div style={row}>
...
function TeamsTab(){
  ...
  const refresh=()=>api('teams').then(j=>setTeams(j.rows||[]))
  useEffect(()=>{refresh()},[])
  useEffect(()=>{ if(teamId) api(`players?team_id=${teamId}`).then(j=>setPlayers(j.rows||[])); else setPlayers([])},[teamId])
  const addTeam=async()=>{ await api('teams',{method:'POST',body:JSON.stringify({name:tName})}); setTName(''); refresh() }
  return (
    <>
      <h2>Teams</h2>
      ...
      <PlayersEditor teamId={teamId} players={players} reload={()=>api(`players?team_id=${teamId}`).then(j=>setPlayers(j.rows||[]))}/>
    </>
  )
}
function PlayersEditor({teamId,players,reload}){
  ...
  const add=async()=>{ await api('players',{method:'POST',body:JSON.stringify({...form,team_id:Number(teamId)})}); setForm({full_name:'',gender:'M',base_hc:0}); reload() }
  const del=async(id)=>{ await api(`players/${id}`,{method:'DELETE'}); reload() }
  return (
    <>
      <h3>Players</h3>
      <div style={row}>
...
function FixturesTab(){
  ...
  useEffect(()=>{ api('seasons').then(j=>setSeasons(j.rows||[])); api('teams').then(j=>setTeams(j.rows||[])) },[])
  useEffect(()=>{ if(seasonId) api(`match-days?season_id=${seasonId}`).then(j=>setMds(j.rows||[])); setMdId('') },[seasonId])
  useEffect(()=>{ if(seasonId) api(`fixtures-manage?season_id=${seasonId}`).then(j=>setFx(j.rows||[])) },[seasonId])
  ...
  const add=async()=>{
    ...
    api(`fixtures-manage?season_id=${seasonId}`).then(j=>setFx(j.rows||[]))
  }
...
function FixtureRow({f,teams}){
  ...
  useEffect(()=>{ api(`player-order?fixture_id=${f.id}`).then(j=>setOrder(j.rows||[])) },[f.id])
  useEffect(()=>{
    const leftTeam = teams.find(t=>t.name===f.left_team)
    const rightTeam = teams.find(t=>t.name===f.right_team)
    if (leftTeam) api(`players?team_id=${leftTeam.id}`).then(j=>setPlayersL(j.rows||[]))
    if (rightTeam) api(`players?team_id=${rightTeam.id}`).then(j=>setPlayersR(j.rows||[]))
  },[f.id,teams])
  ...
  const save=async()=>{ await api('player-order',{method:'POST',body:JSON.stringify({fixture_id:f.id,rows:order})}) }
  return (
    <tr>
